...
static void part2()
{
    ...
    // Console.WriteLine(visibleTrees);
    double[,] treeHeights = new double[numRows, numCols];
    int[,] visibleTreesMap = new int[numRows, numCols];
    (int, int)[] steps = new[] {
        (0, 1), // Right
        (0, -1), //Left
        (1, 0), // Down
    ...
    for (int i=1; i < numRows-1; i++)
    {
        for (int j=1; j < numCols-1; j++)
        {
            visibleTreesMap[i, j] = CountVisible(i, j, numRows, numCols, treeHeights);
            maxVisible = Math.Max(maxVisible, visibleTreesMap[i, j]);
        }
    }
    ...
    static int CountVisible(int row, int col, int numRows, int numCols, double[,] treeHeights)
    {
        return CountVisibleLeft(row, col, treeHeights) *
        CountVisibleRight(row, col, numCols, treeHeights) *
        CountVisibleTop(row, col, treeHeights) *
        CountVisibleBottom(row, col, numRows, treeHeights);
    }

    static int CountVisibleLeft(int row, int col, double[,] treeHeights)
    {
        int count = 0;
        for (int i=col-1; i >= 0; i--)
        {
    ...
    static int CountVisibleRight(int row, int col, int numCols, double[,] treeHeights)
    {
        int count = 0;
        for (int i=col+1; i < numCols; i++)
        {
    ...
    static int CountVisibleTop(int row, int col, double[,] treeHeights)
    {
        int count = 0;
        for (int i=row-1; i >= 0; i--)
        {
    ...
    static int CountVisibleBottom(int row, int col, int numRows, double[,] treeHeights)
    {
        int count = 0;
        for (int i=row+1; i < numRows; i++)
        {
